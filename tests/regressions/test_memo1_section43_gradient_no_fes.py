...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "memo 1 §4.3: `_compute_first_difference_markups` gated on "
        "`self._absorb_cost_ids is not None`; gradient is zero when "
        "there are no cost fixed effects, so `demand_adjustment=True` "
        "silently reduces to the unadjusted variance"
    )
)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "memo 1 §4.3: same bug affects TRV denominator; without the fix, "
        "TRV with demand_adjustment=True equals TRV with demand_adjustment=False"
    )
)
