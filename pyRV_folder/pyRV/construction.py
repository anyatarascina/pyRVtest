...
def build_markups(products: RecArray, demand_results: Mapping, model_downstream: str, ownership_downstream: Array,
    model_upstream: Optional[str] = None,  ownership_upstream: Optional[Array] = None, vertical_integration: Optional[Array] = None)-> Array:
    ...
    markups_downstream = np.zeros((N,1))
    markups_upstream = np.zeros((N,1))
    ...
    for mm in mkts:
        ...
        #Compute downstream markups
        O_mm = ownership_downstream[ind_mm]
        O_mm = O_mm[:, ~np.isnan(O_mm).all(axis=0)]

        if model_downstream == 'bertrand':
            markups_mm = -inv(O_mm*dsdp)@s
        elif model_downstream == 'cournot':
            markups_mm = -(O_mm*inv(dsdp))@s
        elif model_downstream == 'monopoly':
            markups_mm = -inv(dsdp)@s
        markups_downstream[ind_mm] = markups_mm 

        #Compute upstream markups (if applicable) following formula in Villas-Boas (2007)       
        if not ownership_upstream is None:
            Ou_mm = ownership_upstream[ind_mm]
